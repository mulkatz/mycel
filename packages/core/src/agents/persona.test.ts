...
describe('createPersonaNode', () => {
    ...
    const mockLlm: LlmClient = {
      invoke: vi.fn().mockResolvedValue({
        content: JSON.stringify({
          response: '1732, wow! Do you know the exact period?',
  ...
  it('should include all gaps in system prompt', async () => {
    const mockLlm: LlmClient = {
      invoke: vi.fn().mockResolvedValue({
        content: JSON.stringify({
          response: 'Interesting! When was this exactly?',
  ...
  it('should generate warm closing when no gaps exist', async () => {
    const mockLlm: LlmClient = {
      invoke: vi.fn().mockResolvedValue({
        content: JSON.stringify({
          response: 'Great, we have a really good picture now! Anything else you want to share?',
          followUpQuestions: [],
        }),
      }),
    };
    ...
    const calls = (mockLlm.invoke as ReturnType<typeof vi.fn>).mock.calls as Array<
      [{ systemPrompt: string }]
    >;
    expect(calls[0][0].systemPrompt).toContain('warm, brief closing');
  });

  it('should pass through all follow-up questions from LLM response', async () => {
    const mockLlm: LlmClient = {
      invoke: vi.fn().mockResolvedValue({
        content: JSON.stringify({
          response: 'Interesting, what time period was this?',
          followUpQuestions: ['Q1?', 'Q2?', 'Q3?', 'Q4?', 'Q5?'],
        }),
      }),
    };
    ...
    // All follow-up questions should be passed through for UI use
    expect(result.personaOutput?.result.followUpQuestions).toHaveLength(5);
  });

  it('should instruct persona to ask at most one question in response text', async () => {
    const mockLlm: LlmClient = {
      invoke: vi.fn().mockResolvedValue({
        content: JSON.stringify({
          response: 'Test',
    ...
    >;
    const prompt = calls[0][0].systemPrompt;
    expect(prompt).toContain('AT MOST ONE follow-up question');
    expect(prompt).toContain('NEVER repeat back what the user just said');
